...
*/
async function traeEntitlements(token, deviceId, fetchFn = fetch) {
	const summary = flat(await post(ENTITLEMENTS_PATH, token, deviceId, fetchFn))["usage_summary"] ?? {};
	...
	async ensureTodayOne(service) {
		...
		const task = (async () => {
			try {
				...
				const runtime = this.runtime[service];
				if (!(service === "workbuddy" ? Boolean(runtime.resolved || runtime.credentials?.token) : Boolean(runtime.credentials?.token))) return;
